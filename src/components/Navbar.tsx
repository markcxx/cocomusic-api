...
export default function Navbar() {
  ...
  const isActive = (path: string) => pathname === path ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100";
    ...
    <nav className="flex items-center justify-between px-6 md:px-10 h-20 border-b border-zinc-200 dark:border-zinc-800/50 sticky top-0 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md z-50">
      <div className="flex items-center gap-8">
        ...
        <div className="hidden md:flex gap-6 text-sm font-medium">
          ...
          <Link href="/playground" className={isActive('/playground')}>调试台</Link>
          <Link href="/docs" className={isActive('/docs')}>文档</Link>
          <Link href="/dashboard" className={isActive('/dashboard')}>运行状态</Link>
        </div>
      </div>
      <div className="flex items-center gap-4">
        ...
        <Link href="/playground" className="bg-zinc-900 dark:bg-white text-white dark:text-zinc-950 text-sm font-bold px-5 py-2.5 rounded-full shadow-lg hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors">立即开始</Link>
      </div>
    </nav>
  );
}
